    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if config.mixup is True:
            ...
            if not config.cskd:
                ...
                if batch_size % 2 == 1:
                    # drop last sample for cs_kd loss
                    target = target[:-1]
                    images = images[:-1]
                target = target[:batch_size // 2]
                feat_ = model(images[:batch_size // 2])
                output = classifier(feat_)
                loss = torch.mean(criterion(output, target))
                with torch.no_grad():
                    feat_cskd = model(images[batch_size // 2:])
                    output_cskd = classifier(feat_cskd)
                if output.shape != output_cskd.shape:
                    print('shape error')

                cskd_loss = kdloss(output, output_cskd.detach())
                loss += config.lamda * cskd_loss
...
def validate(val_loader, model, classifier, criterion, config, logger):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.3f')
    top1 = AverageMeter('Acc@1', ':6.3f')
    top5 = AverageMeter('Acc@5', ':6.3f')
